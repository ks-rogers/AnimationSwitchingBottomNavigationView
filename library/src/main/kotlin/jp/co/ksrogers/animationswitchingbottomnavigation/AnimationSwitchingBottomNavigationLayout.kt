...
class AnimationSwitchingBottomNavigationLayout @JvmOverloads constructor(
  ...
  override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
    ...
    val navigationWidth = navigationView.measuredWidth
    val navigationHeight = navigationView.measuredHeight
    if (navigationWidth != this.measuredWidth) {
      Log.d(
        TAG_NAME,
        "Nav width is not same as this width. nav=($navigationWidth, $navigationHeight) and this=($measuredWidth, $measuredHeight)"
      )
    }
    if (navigationHeight > navigationViewHeight) {
    ...
    val contentHeightSpec = MeasureSpec.makeMeasureSpec(
      measuredHeight - navigationView.measuredHeight,
      MeasureSpec.EXACTLY
    )
    content.measure(contentWidthSpec, contentHeightSpec)
    Log.d(
      TAG_NAME,
      "NavView(width=${navigationView.measuredWidth}, height=${navigationView.measuredHeight})"
    )
    Log.d(TAG_NAME, "NavLayout(width=$measuredWidth, height=$measuredHeight)")
  }

  override fun onLayout(changed: Boolean, left: Int, top: Int, right: Int, bottom: Int) {
    // superを呼ぶとすでに子ビューにはparamsが設定されている
    ...
    Log.d(
      TAG_NAME,
      "selected(left=${selectedButton.left}, top=${selectedButton.top}, right=${selectedButton.right}, bottom=${selectedButton.bottom})"
    )
